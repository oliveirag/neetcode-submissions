...
def check_keys(hash_set: Set[str], keys: List[str]) -> List[bool]:
    ...
    for i in range(len(keys)): # loop thru each key
        if keys[i] in hash_set: # if the key is in the hash set
            exists.append(True) # add true to the list
        else:                   # if key is not in hash set
            exists.append(False) # add false to the list
    return exists # return list of booleans
