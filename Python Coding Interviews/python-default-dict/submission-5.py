...
def nested_list_to_dict(nums: List[List[int]]) -> Dict[int, List[int]]:
    ...
    for sublist in nums: # for each sublist in nums
        key = sublist[0] # set the key as the first element in each sublist

        for i in range(1, len(sublist)): # loop thru each element in each sublist (skipping the key)
            d[key].append(sublist[i]) # d[key] creates the empty list and .append adds each element

    return d # return a dictionary that has the first element of each sublist as the key
