...
def get_values(hash_map: Dict[str, int], keys: List[str]) -> List[int]:
    ...
    for string in keys: # loop through each string in keys list
        result.append(hash_map[string]) # add each key that is in both hash_map and keys
    return result                    # to the end of the list
